...
export class Grafik {
	...
	crtajGrafik() {
		...
		//http://localhost:8080/Konobar/PreuzmiStatistikuKonobara/1/12-29-21
		//console.log(datum);

		let idKafica = this.container.querySelector('.kafic').value;
		//console.log(idKafica);
		fetch(
			'http://localhost:8080/Konobar/PreuzmiStatistikuKonobara/' +
				idKafica +
				'/' +
				datum,
			{
			...
			if (s.ok) {
				...
				new ProizvodModal(document.body).prikazi(
					null,
					'Greška',
					'Doslo je do greske prilikom ucitavanja statistike!'
				);
			}
		});
	}
	...
	crtajStubice() {
		let selektovani = this.container.querySelectorAll(
		...
		this.podaci.forEach((stats) => {
			if (stats.dnevnica > maxDnevnice) maxDnevnice = stats.dnevnica;
			if (stats.ukupno > maxUkupno) maxUkupno = stats.ukupno;
			if (stats.napojnice > maxNapojnice) maxNapojnice = stats.napojnice;
