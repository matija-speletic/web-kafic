...
export class Kafic {
	...
	crtajKafic(host) {
		...
		this.listaNarucenih.container
			.querySelector('.okvir_liste')
			.classList.add('okvir_liste_novi');
		...
		fetch('http://localhost:8080/Kafic/PreuzmiStolove/' + this.id, {
			method: 'GET',
		}).then((s) => {
			if (s.ok) {
				//console.log(s);
		...
		const getUpdates = () => {
			...
			fetch(
				`http://localhost:8080/Kafic/GetUpdates/${this.id}/${this.lastUpdate}`
			).then((s) => {
				if (s.ok) {
					s.json().then((updates) => {
						if (updates.length > 0) {
	...
	izvrsiNarudzbinu() {
		if (this.selektovanaNarudzbinaID < 0) {
			new ProizvodModal(document.body).prikazi(
				null,
				'Upozorenje',
				'Morate selektovati zauzet sto!'
			);
			return;
		}
		fetch(
			'http://localhost:8080/Narudzbina/IzvrsiNarudzbinu/' +
				this.selektovanaNarudzbinaID +
				'/' +
				this.selektovaniSto.id,
			{
				method: 'PUT',
			}
		).then((s) => {
	...
	obrisiNarudzbinu() {
		...
		fetch(
			'http://localhost:8080/Narudzbina/ObrisiNarudzbinu/' +
				this.selektovanaNarudzbinaID +
				'/' +
				this.selektovaniSto.id,
			{
	...
	dodajNarudzbinu() {
		if (
			this.selektovaniSto == null ||
			this.selektovaniSto.slobodan == false
		) {
			new ProizvodModal(document.body).prikazi(
				null,
				'Upozorenje',
				'Morate izabrati slobodan sto'
			);
			return;
		}
		let nadimakKonobara =
			this.container.querySelector('.konobar_nadimak').value;
		let stranicaZaDodavanje =
			'http://localhost:5500/Client/orderPage.html?nazivKafica=' +
			this.naziv +
			'&x=' +
			this.selektovaniSto.x +
			'&y=' +
			this.selektovaniSto.y;
		if (nadimakKonobara !== '')
	...
	ocistiKontrole() {
		this.container.querySelector('.ukupno .ukupan_iznos').innerHTML =
			'0.00';
		this.container.querySelector('.napojnica .ukupan_iznos').innerHTML =
			'0.00';
		this.listaNarucenih.isprazniListu();
		this.container.querySelector('.lista_proizvoda').innerHTML =
			'Sto je slobodan...';
		this.container.querySelector('.uputstvo_labela').innerHTML =
			'Uputstvo: ';
	...
	ucitajKonobare(konobarSelect) {
		fetch('http://localhost:8080/Konobar/PreuzmiKonobare/' + this.id, {
			method: 'GET',
		}).then((s) => {
			if (s.ok) {
