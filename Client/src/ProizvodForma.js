...
export class ProizvodForma {
	...
	crtajFormu(host) {
		...
		this.listaProizvoda.crtajListu(
			host,
			false,
			this.prikaziProizvod,
		...
		dugmeIzmeni.addEventListener('click', () => {
			this.izmeniProizvod(
				this.listaProizvoda.selektovaniProizvod,
				this.listaProizvoda.listaKategorija
			);
		});
		...
		dugmeUkloni.addEventListener('click', (event) => {
			if (
				this.listaProizvoda.selektovaniProizvod == null ||
				this.listaProizvoda.selektovaniProizvod == undefined
			)
				new ProizvodModal(document.body).prikazi(
					null,
					'Upozorenje',
					'Morate selektovati proizvod!'
				);
			else {
		...
		dugmeNazad.onclick = () => {
			window.open(
				'http://localhost:5500/Client/pocetniMeniPage.html',
				'_self'
			);
		};
	...
	crtajSelectZaJedinicu(spanZaKolicinu) {
		...
		fetch('http://localhost:8080/Proizvod/PreuzmiJedinice', {
			method: 'GET',
		}).then((s) => {
			if (s.ok) {
	...
	prikaziProizvod(proizvod) {
		let opcijaJedinica = document.querySelector(
			'option[value="' + proizvod.kategorija + '"]'
		);
		document.querySelector('.kategorija_select').value =
			opcijaJedinica.value;
	...
	dodajProizvod() {
		...
		let kategorijaID =
			this.container.querySelector('.kategorija_select').value;
		let opis = this.container.querySelector('textarea').value;
		let postRequest = `http://localhost:8080/Proizvod/DodajProizvod?naziv=${naziv}&cena=${parseInt(
			cena
		)}&kolicina=${kolicina}&jedinica=${jedinica}&opis=${opis}&idKategorije=${kategorijaID}`;
		...
		fetch(postRequest, { method: 'POST' }).then((s) => {
			if (s.ok) {
				...
				s.json().then((idDodatog) => {
					if (
						this.container.querySelector('.kategorija').value ==
						kategorijaID
					) {
						let noviProizvod = new Proizvod(
							idDodatog,
							naziv,
							parseInt(cena),
							kategorijaID,
	...
	izmeniProizvod(selektovaniProizvod, listaKategorija) {
		if (selektovaniProizvod == null || selektovaniProizvod == undefined) {
			new ProizvodModal(document.body).prikazi(
				null,
				'Upozorenje',
				'Morate prethodno selektovati neki proizvod!'
			);
		} else {
			let naziv =
			...
			let cena = this.container.querySelector('.cena_input').value;
			let kolicina =
				this.container.querySelector('.kolicina_input').value;
			let jedinica =
				this.container.querySelector('.jedinica_input').value;
			let kategorijaID =
				this.container.querySelector('.kategorija_select').value;
			let kategorijaNaziv = listaKategorija.find(
				(k) => k.id == kategorijaID
			).naziv;
			let opis = this.container.querySelector('textarea').value;
			var zaSlanje = {
			...
			fetch('http://localhost:8080/Proizvod/PromeniProizvod', {
				method: 'PUT',
				...
				if (s.ok) {
					...
					this.listaProizvoda.ucitajProizvode(
						kategorijaID,
						false,
						this.prikaziProizvod
					);
	...
	obrisiProizvod(idProizvoda) {
		fetch('http://localhost:8080/Proizvod/ObrisiProizvod/' + idProizvoda, {
			method: 'DELETE',
		}).then((s) => {
			if (s.ok) {
				new ProizvodModal(document.body).prikazi(
				...
				//window.location.reload();
				let proizvod = this.listaProizvoda.proizvodi.find(
					(p) => p.id == idProizvoda
				);
				this.listaProizvoda.listaContainer.removeChild(
					proizvod.container
				);
			} else
				new ProizvodModal(document.body).prikazi(
