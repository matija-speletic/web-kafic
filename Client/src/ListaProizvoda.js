...
export class ListaProizvoda {
	...
	ucitajKategorije(
		...
		fetch('http://localhost:8080/Proizvod/PreuzmiKategorije', {
			method: 'GET',
		}).then((s) => {
			if (s.ok) {
				s.json().then((data) => {
					...
					data.forEach((kategorija) => {
						...
						this.listaKategorija.push(
							new Kategorija(kategorija.id, kategorija.naziv)
						);
					});
					if (!saKontrolama) {
	...
	ucitajProizvode(kategorijaID, saKontrolama = true, naKlik = undefined) {
		fetch(
			'http://localhost:8080/Proizvod/PreuzmiProizvode/' + kategorijaID,
			{
				method: 'GET',
			}
		).then((s) => {
			if (s.ok) {
				s.json().then((data) => {
					...
					data.forEach((proizvod) => {
						...
						if (saKontrolama) {
							izabraniProizvod =
								this.narudzbinaForma.listaNarucenih.naruceniProizvodi.filter(
									(p) => p.id === proizvod.id
								);
							if (izabraniProizvod.length === 0)
								...
								brojIzabranih =
									izabraniProizvod[0].brojIzabranih;
						}
						this.proizvodi.push(
							new Proizvod(
								proizvod.id,
